...
export function ContinueCard({
  ...
  return (
    <article className="flex w-[223px] shrink-0 flex-col gap-3 overflow-hidden rounded-[12px] bg-surface p-1.5 lg:w-[293px] lg:p-1">
      <CourseThumb image={item.image} seed={item.courseId} className="w-full" />
      <div className="px-1">
        <p className="truncate text-[14px] font-medium leading-[18px] text-text">{item.name}</p>
      </div>
      <div className="flex w-full flex-col gap-2 overflow-hidden rounded-[8px] bg-surface px-2.5 py-2">
        <div className="flex items-center justify-between text-[10px] font-medium">
          <p className="text-text">{labels.percentCompleted.replace("{n}", String(item.pct))}</p>
          <p className="text-muted">{labels.hrsLeft.replace("{n}", String(item.hrsLeft))}</p>
        </div>
        <div className="flex h-[6px] items-center gap-[3px]">
          {Array.from({ length: 6 }, (_, i) => (
            <span key={i} className="h-full min-w-px flex-1 overflow-hidden rounded-[3px] bg-surface-2">
      ...
      <div className="flex items-center justify-end gap-[10px] px-1 pb-1">
        <p className="min-w-0 flex-1 text-[10px] leading-normal text-muted">
          {labels.lastStudied}
          {item.lastStudied ? (
            <>
              <br />
        ...
        >
          <span className="text-[12px] font-medium text-text">{labels.resume}</span>
