...
export function LanguageSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="absolute inset-x-0 bottom-0 rounded-t-[24px] bg-surface pb-[calc(env(safe-area-inset-bottom)+20px)] pt-5">
        <div className="mx-auto mb-4 h-0.5 w-[30px] rounded-full bg-white/30" />
        <p className="px-[19px] text-[16px] font-semibold text-text">{title}</p>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? "border-white/40 text-text" : "border-line text-text",
      )}
    >
      {label}
